# -*- coding: utf-8 -*-
"""
Created on Tue Oct 13 17:48:39 2020
...
#    age_json = 34
  ...
  #  job_json = 'job_admin'
    ...
    url_test['job_blue-collar'] = pd.DataFrame([job_blue_collar_value])
    ...
    url_test['job_self-employed'] = pd.DataFrame([job_self_employed_value])
 ...
 #   balance_json = 869
    ...
    url_test['balance'] = pd.to_numeric(url_test['balance'])
  #  url_test['balance'] = scaler_model.transform((url_test['balance']))
    
    campaign_json = request.args.get('campaign') 
  #  campaign_json = 1
    print('campaign_json:---------------->', pd.to_numeric(campaign_json))
    url_test['campaign'] = pd.DataFrame([campaign_json])
    url_test['campaign'] = pd.to_numeric(url_test['campaign'])
    
    duration_json = request.args.get('duration') 
   # duration_json = 1677
    print('duration_json:---------------->', (duration_json))
    url_test['duration'] = pd.DataFrame([duration_json])
    url_test['duration'] = pd.to_numeric(url_test['duration'])
    #url_test['duration'] = scaler_model.transform((url_test['duration']))
    
    day_json = request.args.get('day')
 #   day_json = 6
    print('day_json:---------------->', (day_json))
    url_test['day'] = pd.DataFrame([day_json])
    url_test['day'] = pd.to_numeric(url_test['day'])
    
    pdays_json = request.args.get('pdays') 
#    pdays_json = -1
    print('pdays_json:---------------->', (pdays_json))
    url_test['pdays'] = pd.DataFrame([pdays_json])
    url_test['pdays'] = pd.to_numeric(url_test['pdays'])
 ...
 #   previous_json = 0
    ...
    url_test['previous'] = pd.to_numeric(url_test['previous'])
  ...
  #  poutcome_json = 'poutcome_unknown'
    ...
    x = url_test[['job_admin','job_blue-collar','job_entrepreneur','job_housemaid',
